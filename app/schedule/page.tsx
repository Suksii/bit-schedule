...
export default async function SchedulePage({
  ...
  function renderRows(slots: (typeof SHIFT_SLOTS)[number][]) {
    return slots.map((slot) => (
      <tr key={slot.key}>
        <td className="border border-gray-400 px-1 md:px-3 py-5 font-bold whitespace-nowrap text-center text-gray-800 bg-gray-50">
          {slot.label}
        </td>
                  ...
                  {cell.map((a) => {
                    ...
                    return (
                      <div key={a.id}>
                        {hasCustomTime && (
                          <div
                            className={`font-semibold leading-tight ${THEME.name.accent}`}
    ...
    <div className="min-h-screen bg-gray-50 print:bg-white">
      ...
      <div className={`${THEME.subheader.bg} px-4 py-2 print:hidden`}>
        <div className="max-w-6xl mx-auto flex items-center gap-4">
          {!isCurrentWeek && (
            <Link
              href={`/schedule?week=${currentWeek}`}
              className={THEME.subheader.navBtn}
            >
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse bg-white text-sm print:text-xs">
            <thead>
              {/* Row 1: day names */}
              <tr
                className={`${THEME.header.row1} print:bg-transparent print:text-black`}
              >
              ...
              {/* Row 2: dates */}
              <tr
                className={`${THEME.header.row2} print:bg-transparent print:text-black`}
              >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                {allEmployees
                  ...
                  .map((emp) => (
                    <div
                      key={emp.id}
                      className="bg-white rounded-lg border border-gray-200 px-4 py-3 flex flex-col gap-1"
                    >
                      ...
                      {emp.phone && (
                        <a
                          href={`tel:${emp.phone}`}
                          className="text-xs text-gray-500 hover:text-gray-700 tabular-nums"
                        >
                          {emp.phone}
                        </a>
                      )}
              ...
              <table className="mx-auto text-xs">
                <tbody>
                  {allEmployees
                    ...
                    .map((emp) => (
                      <tr key={emp.id}>
                        <td className="pr-4 py-0.5 text-right font-semibold">
                          {emp.name}
                        </td>
                        <td className="pr-6 py-0.5 tabular-nums">
                          {emp.phone ?? ""}
                        </td>
